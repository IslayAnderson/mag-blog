                ...
                <?php if (have_posts()) : while (have_posts()) : the_post(); ?>
                    ...
                    <article id="post-<?php the_ID(); ?>" <?php post_class('cf'); ?> role="article">
                        ...
                        <div class="govuk-inset-text">
                            <?php printf(__
                                ('Posted', 'mag_blog_theme') . ' %1$s %2$s',
                                '<time class="updated entry-time" datetime="' . get_the_time('Y-m-d') . '" itemprop="datePublished">' . get_the_time(get_option('date_format')) . '</time>',
                                '<span class="by">' . __('by', 'mag_blog_theme') . '</span> <span class="entry-author author" itemprop="author" itemscope itemptype="http://schema.org/Person">' . get_the_author_link(get_the_author_meta('ID')) . '</span>'
                            ); ?>
                        </div>
                    ...
                    if ($comments) {
                        foreach ($comments as $comment) {
                            mag_blog_theme_comments($comment);
                        }
                    } else {
                        mag_blog_theme_error('No comments found.');
                    }
                    ...
                    <div id="respond" class="comment-respond">
                        <h3 id="reply-title" class="comment-reply-title">Leave a Reply
                            <small>
                                <a rel="nofollow" id="cancel-comment-reply-link"
                                   href="/socialchaff-an-exercise-in-obfuscation-by-hiding-in-plain-sight/#respond"
                        ...
                        <form action="/wp-comments-post.php" method="post" id="commentform" class="comment-form">
                            <div class="govuk-form-group">
                                ...
                                <textarea class="govuk-textarea" id="comment" name="comment" rows="5"
                                          aria-describedby="more-detail-hint"></textarea>
                            </div>
                            <div class="govuk-form-group">
                                <label class="govuk-label" for="full-name">User name</label>
                                <input class="govuk-input" id="author" name="author" type="text" spellcheck="false"
                                       autocomplete="name">
                            </div>
                            <div class="govuk-form-group">
                            ...
                            <div class="govuk-form-group">
                                ...
                                <input class="govuk-input" id="url" name="url" type="text" spellcheck="false"
                                       autocomplete="url">
                            </div>
                            <input name="submit" type="submit" id="submit" class="govuk-button"
                                   data-module="govuk-button" value="Post Comment">
